...
- **Native AOT compilation (`qc`)** — **ACTIVE, 0.0.65**. x86-64 only
  (AArch64 backend deferred POST-1.0).
  Bootstraps via a **true 2-stage self-host**: the committed `bin/x86/qc`
  compiles the 0.0.65 source to a faithful `qc` (verified byte-identical to
  the golden binary, 96/96 gate). Passes **96/96** test_suites
...
`compiler/0.0.65/src/x86/` (`main.quanta` + `helpers`/`lexer`/`parse`/
...
```bash
# 2-stage self-host: committed bin/x86/qc compiles the 0.0.65 source to a faithful qc
cd compiler/0.0.65/src/x86
SEED=../../../../compiler/0.0.65/bin/x86/qc
