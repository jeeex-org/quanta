...
- **Native AOT compilation (`qc`)** — **ACTIVE, 0.0.66**. x86-64 only
  (AArch64 backend deferred POST-1.0).
  Bootstraps via a **true 2-stage self-host**: the committed `bin/x86/qc`
  compiles the 0.0.66 source to a faithful `qc` (verified byte-identical to
  the golden binary, 99/99 gate). Passes **99/99** test_suites
...
`compiler/0.0.66/src/x86/` (`main.quanta` + `helpers`/`lexer`/`parse`/
...
```bash
# 2-stage self-host: committed bin/x86/qc compiles the 0.0.66 source to a faithful qc
cd compiler/0.0.66/src/x86
SEED=../../../../compiler/0.0.66/bin/x86/qc
