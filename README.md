...
- **Native AOT compilation (`qc`)** — **ACTIVE, 0.0.68**. x86-64 only
  (AArch64 backend deferred POST-1.0).
  Bootstraps via a **true 2-stage self-host**: the committed `bin/x86/qc`
  compiles the 0.0.68 source to a faithful `qc` (verified byte-identical to
  the golden binary, 105/105 gate). Passes **105/105** test_suites
...
`compiler/0.0.68/src/x86/` (`main.quanta` + `helpers`/`lexer`/`parse`/
...
```bash
# 2-stage self-host: committed bin/x86/qc compiles the 0.0.68 source to a faithful qc
cd compiler/0.0.68/src/x86
SEED=../../../../compiler/0.0.68/bin/x86/qc
