...
| **0.0.125+** | 0.0.125 → 0.0.134 | **Cores resume at 0.0.125** (one feature per version, fixpoint-verified, per 2026-08-30 directive — cores do NOT go into 0.1.0). Sequencing: `time` (0.0.125) → `process` fork/exec/waitpid (0.0.126) → `big` div-by-zero guard FIX-0.0.19 (0.0.127) → generics type constraints FIX-0.0.33 (0.0.128) → `json` (0.0.129) → `secure` TLS 1.3 (0.0.130) → `http` (0.0.131) → `quic` UDP+TLS (0.0.132) → PTY layer (0.0.133, needs 0.0.126 process) → borrow-check (0.0.134, language safety pass). Each self-hosts + gate-green before promotion. |
...
AFTER all 0.0.125–0.0.134 cores are done (0.1.0 = post-core STABLE).
...
**0.0.125→0.0.134 (core sequence, per 2026-08-30 — cores do NOT go into 0.1.0):**
- 0.0.125 `time` (clock/now/sleep/nanosleep)
- 0.0.126 `process` (fork/exec/waitpid — today only via `$$()` raw-syscall)
- 0.0.127 `big` div-by-zero guard (FIX-0.0.19, real hang bug)
- 0.0.128 generics type constraints (FIX-0.0.33)
- 0.0.129 `json` stdlib
- 0.0.130 `secure` (TLS 1.3)
- 0.0.131 `http`
- 0.0.132 `quic` (UDP+TLS)
- 0.0.133 PTY layer (needs 0.0.126 process + pty-alloc) — core
- 0.0.134 borrow-check (language safety pass) — core
