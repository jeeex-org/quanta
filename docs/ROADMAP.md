...
### Current status (0.0.116 — live)

**0.0.116 (current stable seed):** AUDIT_ROADMAP round-2 close-off. FIX-0.0.31–35 + FIX-0.0.40–45 all closed: rsp() ModR/M no-op miscompile, hex-literal 64-bit range, sin/cos/tan wrong-vreg reload (now bit-exact vs libm), quantum Keccak rho+pi + sponge rewrites (verified vs OpenSSL), linalg mat_from_flat off-by-8 + mat_det Bareiss rewrite. New gate tests: quantum_test, linalg_test, trig_test (EXPECTED.tsv 151 rows). MULTI-TU gate layer wired (mtu_* fixtures). Full 10-layer gate GREEN (151/151 functional + 7/7 stdlib + 3/3 multi-tu + extern-c/security/perf/valgrind/fuzz/differential/generics). **Self-host fixpoint BYTE-VERIFIED** (md5 `662de43a69d848581774e81f01703456`). Remaining cores before 0.1.0 (no-deferral policy, user directive 2026-08-28): 0.0.117 (`big` completion: ordering + bitwise ops), 0.0.118 (net send/recv), 0.0.119 (futex + threads), 0.0.120 (stack_trace full unwind), 0.0.121 (closures by-ref capture), 0.0.122 (extern "C" standalone EXE). Then 0.1.0 (std/lib resumes, borrow-checking, PTY layer). P4 tooling moved AFTER stdlibs (POST-0.1.0).
