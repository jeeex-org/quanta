...
> **Last updated: 2026-08-29. Current compiler: 0.0.119** (x86-64 ELF emitter,
...
### Current status (0.0.119 — live)

**0.0.119 (current stable seed):** concurrency completion — `futex_wait`/`futex_wake` (sc 202) + `thread_create(fn,arg)`/`thread_join(tid)` (clone sc 56 + per-thread 1MB stack mmap + join-slot mmap; CLONE_CHILD_CLEARTID wakes join on child exit). `thread_test.quanta` (worker(7)=107, join returns 107) + `futex_test.quanta` GREEN. Remaining cores before 0.1.0 (no-deferral policy, user directive 2026-08-28): 0.0.120 (stack_trace full unwind), 0.0.121 (closures by-ref capture), 0.0.122 (extern "C" standalone EXE). Then 0.1.0 (std/lib resumes, borrow-checking, PTY layer). P4 tooling moved AFTER stdlibs (POST-0.1.0).
