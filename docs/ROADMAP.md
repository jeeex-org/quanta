...
> **Last updated: 2026-08-30. Current compiler: 0.0.122** (x86-64 ELF emitter,
...
| **P4 tooling** | POST-0.1.0 | **Quanta-native code-writing tool** (edit Quanta source reliably without external scripting — the user's stated goal). **Moved AFTER the stdlibs stage** per user directive 2026-08-28 (was: last core item before 0.1.0). No version reserved. |
| **0.1.0** | 0.1.0 | Core + builtins complete → std/lib resumes; borrow-checking target for #1 green; **PTY layer for interactive `$$()` (vi/ssh/top)**. Preceded by 0.0.116–0.0.122 (audit close-off #2, `big` completion, net send/recv, futex+threads, stack_trace unwind, closures by-ref, extern-C standalone EXE). |
...
| 0.0.122 | Linker | extern "C" standalone EXE (PLT/GOT) | ✅ done | **Standalone extern-C EXE without gcc (0.0.108 was object-mode+gcc only).** A Quanta program using `extern "C"` now links libc symbols STANDALONE with no gcc in the toolchain: `qc prog out --emit-obj` emits a relocatable ELF64 `.o` carrying `R_X86_64_PLT32` UNDEF relocs for each extern call; the system linker `ld` emits the PLT/GOT + `PT_INTERP`, and `scripts/quanta_link.sh` drives `ld` (probing `/lib64/ld-linux-x86-64.so.2`, `-lc`) so PT_INTERP uses the Linux glibc interpreter (ld's built-in default is the BSD name `/lib/ld64.so.1`, which does not exist on glibc → "required file not found"). This satisfies ROADMAP's "emit PLT/GOT so a Quanta EXE links libc standalone without gcc" — the linker emits the PLT/GOT, qc only emits the relocs + `_start`/`__init`. **Fixpoint trap (verified this version):** hand-rolling the dynamic ELF (PT_INTERP/PT_DYNAMIC/GOT/PLT) inside `write_elf` was attempted and REJECTED — it breaks the self-host fixpoint in every placement (B≠C byte-identical fails; qc's own build miscompiles), and shelling out to `ld`/`gcc` from compiler source also breaks B==C. The object-mode + external-`ld` path keeps `write_elf` static, so the qc self-host is untouched. String-arg header skip (`char*` = data+8) for extern calls is GREEN (strlen("hello")→5, printf/puts/puts("EXTERN_C_OK") flush via libc exit()). Gated: `extern_c_ffi.quanta` links under BOTH `gcc -nostartfiles` (extern-c) AND `ld` (extern-ld, GCC-FREE) → both GREEN, sentinel EXTERN_C_OK. **Fixpoint:** 0.0.122 source self-hosts stage1==stage2 byte-identical (md5 `a1babb9aedd01e56b5bcb93487a651af`, seed 0.0.121); golden self-rebuild == golden byte-identical. |
| 0.0.123 | Completeness | known-gaps close-off (closure/extern/security) | ❌ todo | **Closes the 3 known gaps surfaced after the 0.0.122 gate (every verified gap gets a version):** (1) **nested named-fn escape** — `fn name(){}` nested inside another fn and returned/escaped errors `undeclared function: helper` (parser loop: nested fn not registered in scope; `\|x\|` lambdas work). Fix: register nested fns in the enclosing scope and route the escape via the same closure path. (2) **multi-arg extern-C linkage** — `extern "C" fn add2(a:i64,b:i64):i64; add2(3,4)` compiles+links but emits `undefined reference to add2` (no UNDEF reloc for the call); single-arg extern works. Fix: emit the `R_X86_64_PLT32` UNDEF reloc for every extern call regardless of arg count; also model variadic C (`printf(fmt, ...)`). (3) **security KNOWN issues** — the gate prints `KNOWN` (non-blocking) items: garbage-input fuzz crashes the COMPILER (SIGILL/SIGSEGV) and compiler SIGILL on extreme literal MININT-1. Re-triage each as blocking/non-blocking and fix the compiler-robustness crashes. **Fixpoint:** 0.0.123 source must self-host stage1==stage2 byte-identical (seed 0.0.122). `write_elf` stays static (no dynamic-ELF hand-roll). |

**Version-number rule:** literal numbers above are the plan; the SEQUENCING (order + one-feature-per-version + fixpoint-verified) is the contract. If a version needs splitting, the number increments — never skipped, never reused. 0.0.90 is **not reserved**; it is simply the `as`/`raw`/`volatile` cluster above. The final cores are 0.0.115 (security hardening, ✅), 0.0.116 (audit close-off #2), 0.0.117 (`big` completion), 0.0.118 (net send/recv), 0.0.119 (futex + threads), 0.0.120 (stack_trace full unwind), 0.0.121 (closures by-ref capture), 0.0.122 (extern "C" standalone EXE), 0.0.123 (known-gaps close-off: nested named-fn escape, multi-arg extern-C linkage, security KNOWN-issue triage) — **no-deferral policy to 0.1.0 (user directive 2026-08-28)**: every verified gap gets a version. **The code-writing tool is deferred to AFTER stdlibs (post-0.1.0)** — it is no longer the last 0.0.x item. ARM64 and P4 tooling remain POST-0.1.0 per earlier user directives.

**POST-0.1.0 (no version reserved for any):** P4 **Quanta-native code-writing tool** (edit Quanta source reliably without external scripting); `chain`/`secure`/`ai`/`physics` libs; ARM64 backend. (Borrow-checking and the PTY layer are part of 0.1.0 itself, not post-0.1.0.)
...
**0.0.122 (current stable seed):** extern "C" standalone EXE without gcc — `qc prog out --emit-obj` + `scripts/quanta_link.sh` (ld, `-lc`, Linux interpreter) links libc STANDALONE via PLT/GOT; `extern_c_ffi.quanta` greps `EXTERN_C_OK` under both gcc and ld. Fixpoint byte-verified (stage1==stage2, md5 `a1babb9aedd01e56b5bcb93487a651af`). Remaining cores before 0.1.0: NONE — 0.0.122 was the last core. Then 0.1.0 (std/lib resumes, borrow-checking, PTY layer). P4 tooling moved AFTER stdlibs (POST-0.1.0).
