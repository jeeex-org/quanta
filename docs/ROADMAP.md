...
### Current status (0.0.117 — live)

**0.0.117 (current stable seed):** `big` completion. Ordering (`< > <= >=`) + bitwise (`& | ^ << >>`) routed to sign-aware `big_cmp`/`big_and`/`big_or`/`big_xor`/`big_shl_signed`/`big_shr_signed`. **Also fixed: `big_add`/`big_sub`/`big_mul` were MAGNITUDE-ONLY** (negative operands silently miscomputed — e.g. `(-5)+3`→8) — now sign-aware via `big_add_signed`/`big_sub_signed`/`big_mul_signed`; `+ - *` operators route to them. `: big` annotation with int-literal RHS now promotes via `big_from_i64` (previously held a raw i64, crashing any big-op). Gate: **152/152 functional + 7/7 stdlib + 3/3 multi-tu + extern-c/security/perf/valgrind/fuzz/differential/generics (all GREEN)**. New gate test `big_ops_test.quanta`. **Self-host fixpoint BYTE-VERIFIED** (md5 `8b8a1e21573f12b5742a64f695a50b85`). Remaining cores before 0.1.0 (no-deferral policy, user directive 2026-08-28): 0.0.118 (net send/recv), 0.0.119 (futex + threads), 0.0.120 (stack_trace full unwind), 0.0.121 (closures by-ref capture), 0.0.122 (extern "C" standalone EXE). Then 0.1.0 (std/lib resumes, borrow-checking, PTY layer). P4 tooling moved AFTER stdlibs (POST-0.1.0).
