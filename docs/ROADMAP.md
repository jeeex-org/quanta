...
> **Last updated: 2026-08-29. Current compiler: 0.0.118** (x86-64 ELF emitter,
...
### Current status (0.0.118 — live)

**0.0.118 (current stable seed):** net completion — `send(fd,buf,len,flags)` + `recv(fd,buf,len,flags)` builtins (x86-64 implements them as `sendto(44)`/`recvfrom(45)` with NULL addr/addrlen; r8/r9 zeroed to avoid EISCONN/hang). `net_test.quanta` transfers 8 real bytes over a socketpair (rc=0). Remaining cores before 0.1.0 (no-deferral policy, user directive 2026-08-28): 0.0.119 (futex + threads), 0.0.120 (stack_trace full unwind), 0.0.121 (closures by-ref capture), 0.0.122 (extern "C" standalone EXE). Then 0.1.0 (std/lib resumes, borrow-checking, PTY layer). P4 tooling moved AFTER stdlibs (POST-0.1.0).
