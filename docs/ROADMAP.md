...
| 0.0.152 | `secure` | Certificate Transparency — SCT parsing/verification (RFC 6962), log list management, inclusion proof verification, stapled SCT validation | ✅ DONE — SCT parse (single/list), log_id/timestamp/sig, Merkle inclusion/consistency proofs, known log list, stapled SCT stub |
| 0.0.153 | `secure` | OCSP Stapling — server-side OCSP response fetch, caching, stapling in TLS handshake, client-side verification | ✅ DONE — OCSP request/response parse, CertID/RevokedInfo/Good/Unknown status, producedAt/thisUpdate/nextUpdate, nonce extension, stapling cache, signature verification stub |
| 0.0.154 | `secure` | TLS Resumption — session tickets (RFC 5077), PSK (RFC 8446), 0-RTT with replay protection | ✅ DONE — ticket encrypt/decrypt (AES-256-GCM), key rotation, PSK binder computation, early secret derivation, 0-RTT replay cache, ticket serialize/deserialize, OCSP stapling integration |
| 0.0.155 | `secure` | mTLS / SPIFFE — X.509-SVID / JWT-SVID parsing, workload identity, trust domain federation, automatic rotation via SPIRE agent API | ✅ DONE — SPIFFE ID parse/build/validate, X.509-SVID cert SAN extraction, JWT-SVID sub claim parse, SVID create/verify/rotate, trust domain/bundle/federation/workload management, SPIRE agent API, mTLS peer verification |
| 0.0.156 | `secure` | Hardware Security Module — PKCS#11 (Cryptoki) interface, TPM 2.0 (ESAPI), secure enclave (SGX/SEV) attestation, key generation/storage/signing in HSM | ✅ DONE — PKCS#11 full API (slots/tokens/sessions/objects, login, key gen for ML-KEM/ML-DSA/SLH-DSA), TPM 2.0 ESAPI (create/load/sign/quote/attest), SGX/SEV attestation (report/quote), HSM abstraction layer (unified backend, key import/export, cert storage, key derivation, health check) |
| 0.0.157 | `secure` | Key Derivation / Secrets at Rest — Argon2id (RFC 9106), Scrypt, PBKDF2, HKDF (RFC 5869), encrypted keystore (AES-256-GCM + Argon2id), key rotation policies | ✅ DONE — PBKDF2-HMAC-SHA256 (RFC 8018), HKDF-Extract/Expand (RFC 5869), Scrypt (RFC 7914), Argon2id (RFC 9106), encrypted keystore with AES-256-GCM + Argon2id, key rotation policies (daily/weekly/monthly/yearly), secure delete (3-pass overwrite) |
| 0.0.158 | `secure` | Side-Channel Hardening — constant-time comparators (all PQC ops), masking/blinding for ML-KEM/ML-DSA/SLH-DSA, cache-line alignment, timing-safe memory access | ✅ DONE — ct_memcmp/ct_eq32/ct_eq64/ct_select32/ct_cond_copy/ct_memcpy/ct_memset, cache-line aligned alloc/prefetch/flush, masked XOR/AND/sharing (ISW), ML-KEM/ML-DSA/SLH-DSA masked keygen/sign/verify, secure alloc/pool, timing test harness |
| 0.0.159 | `secure` | Audit & Tamper-Evident Logging — Merkle-tree log (RFC 6962), Trillian-compatible, signed checkpoints, inclusion/consistency proofs, sigstore/cosign integration | ✅ DONE — Merkle tree log (RFC 6962) with leaf/node hashes, tree head signing (ML-DSA), inclusion/consistency proofs, Trillian-compatible API, sigstore bundle create/verify, cosign keyless sign/verify, in-toto/SLSA provenance, SPDX/CycloneDX SBOM, reproducible build attestation |
| 0.0.160 | `secure` | Supply Chain Security — in-toto/SLSA provenance, sigstore/cosign signing & verification, reproducible build attestation, SBOM (SPDX/CycloneDX) generation, policy engine, supply chain graph | ✅ DONE — In-toto layout/link verification, SLSA provenance verify (levels 0-3), sigstore/cosign keyless verify, transparency log inclusion, SPDX/CycloneDX SBOM verify/diff, reproducible build fingerprint/verify, attestation create/verify, OPA-like policy eval/compile, dependency graph build/paths/cycles |
| 0.0.161 | `secure` | Compliance Artifacts — FIPS 140-3 Level 1 module boundary, Common Criteria EAL4+ evidence, ISO 27001 control mapping, NIST 800-53 traceability, SOC 2 Type II, GDPR/CCPA privacy controls | ✅ DONE — FIPS 140-3 CM (module boundary, interfaces, roles/services, integrity, env isolation, non-invasive, SSP, self-tests, lifecycle, mitigation), CC EAL4+ (ST: ASE_INT.1-ASE_TSS.1, ADV: ARC.1-FSP.4-IMP.1-TDS.3, AGD: OPE.1-PRE.1, ALC: CMC.4-CMS.4-DEL.1-DVS.1-LCD.1-TAT.1, ATE: COV.2-DPT.2-FUN.1-IND.2, AVA: VAN.3), ISO 27001:2022 (A.5-A.18 mappings), NIST 800-53 Rev 5 (AC/AU/CM/IA/SC/SI/SA families), SOC 2 Type II (CC/A/C criteria), GDPR (Art. 5/25/32/33/35), CCPA (§1798.100/150/198), CNSA 2.0 (ML-KEM-768, ML-DSA-65, SLH-DSA, AES-256, SHA-384) |
| 0.0.162 | `secure` | Policy Engine — OPA/Rego-compatible policy language, runtime evaluation (TLS config, cert pinning, cipher suite enforcement, mTLS requirements), audit trail | ✅ DONE — Rego subset parser (package/import/default/rules/functions), native evaluator for TLS config (min/max version, AEAD ciphers, curves, cert/key), cert pinning (SPKI SHA-256 matching), cipher suites (strict/permissive mode, deny list for weak suites, allow list for TLS 1.3), mTLS (client cert, hostname verify, trust domains, SPIFFE IDs), HSM key usage (PQC keys hw-only/non-extractable, op restrictions), supply chain (SBOM required, SLSA level >= 2, sigstore verify), compliance (framework status/evidence), built-in policy bundle, decision logging to audit log |
