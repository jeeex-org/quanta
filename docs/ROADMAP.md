...
| 0.0.116 | Bugfix+Hygiene | AUDIT_ROADMAP close-off #2 | 🟡 WIP | (1) Fix builtin-result + live-local arithmetic miscompile (`rsp()+z` → `z+z`; pre-existing since ≤0.0.114 — any builtin result used in an arithmetic expr with a live local miscomputes). (2) Add `quantum_test` + `linalg_test` gate tests (ROADMAP noted these libs lack dedicated gate tests) + gate `sin`/`cos`/`tan` (implemented x87 since 0.0.103 but never gated). (3) Wire multi-TU `mtu_*` fixtures into the gate. |
| 0.0.117 | Lang | `big` completion: ordering + bitwise ops | ❌ todo | Route `< > <= >=` on big to a signed compare (currently rejected at compile time, error kind 6); route `& | ^ << >>` to big_and/or/xor/shl/shr (`big_shl`/`big_shr` exist as lib calls only; `big_and`/`big_or`/`big_xor` to be added to lib/std/big.quanta). Post-quantum crypto / hashing need big-int bitwise + compare. Compiler delta is operator routing only; arithmetic stays in lib/std (core-vs-stdlib line). |
| 0.0.118 | Builtins | net completion: `send`/`recv` | ❌ todo | net (0.0.106) shipped socket/connect/bind/listen/accept (sc 41/42/49/50/43) but NO data transfer — `send(fd,buf,len,flags)` (sc 44) + `recv(fd,buf,len,flags)` (sc 45) builtins; extend `net_test` to transfer real bytes over a loopback socketpair. Networking that can't move data is a stub; "builtins complete" requires it. |
| 0.0.119 | Builtins | concurrency completion: futex + threads | ❌ todo | atomics (0.0.105) shipped 5 lock-prefixed ops but no blocking/spawn: `futex_wait(addr,expected)`/`futex_wake(addr,n)` (sc 202) + `thread_create(fn,arg)`/`thread_join(tid)` (clone sc 56 + per-thread stack mmap). Atomics without threads is half a concurrency feature. Design pass: allocator interaction, thread stack layout, join semantics. |
| 0.0.120 | Builtins | `stack_trace` full unwind | ❌ todo | stack_trace (0.0.113) returns ONE frame (immediate caller return address). Extend to walk the frame chain (rbp walk over g_in_frame frames) and print/return the full call stack. |
| 0.0.121 | Lang | closures: by-ref capture | ❌ todo | closure captures (0.0.67) are by-value only (heap env copy). Add by-ref capture so a closure can mutate an enclosing local. Documented v1 limitation — now scheduled, not deferred. |
| 0.0.122 | Linker | extern "C" standalone EXE (PLT/GOT) | ❌ todo | extern "C" (0.0.108) works object-mode+gcc only. Emit PLT/GOT entries so a Quanta EXE links libc symbols standalone without gcc. Last core before 0.1.0. |

**Version-number rule:** literal numbers above are the plan; the SEQUENCING (order + one-feature-per-version + fixpoint-verified) is the contract. If a version needs splitting, the number increments — never skipped, never reused. 0.0.90 is **not reserved**; it is simply the `as`/`raw`/`volatile` cluster above. The final cores are 0.0.115 (security hardening, ✅), 0.0.116 (audit close-off #2), 0.0.117 (`big` completion), 0.0.118 (net send/recv), 0.0.119 (futex + threads), 0.0.120 (stack_trace full unwind), 0.0.121 (closures by-ref capture), 0.0.122 (extern "C" standalone EXE) — **no-deferral policy to 0.1.0 (user directive 2026-08-28)**: every verified gap gets a version. **The code-writing tool is deferred to AFTER stdlibs (post-0.1.0)** — it is no longer the last 0.0.x item. ARM64 and P4 tooling remain POST-0.1.0 per earlier user directives.
...
### Current status (0.0.115 — live)

**0.0.115 (current stable seed):** Security — AUDIT_ROADMAP close-off (hardening). All findings FIX-0.0.1–30 closed (heap guards, realloc header, double-free ownership, include/source bounds, stack-trace guard, owned_stk cap, 16MB source pre-check, big_print_dec_mag overflow fix, stdlib suite wired into gate, pointer-builtin posture documented). Full 9-layer gate GREEN (148/148 functional + 7/7 stdlib + extern-c/security/perf/valgrind/fuzz/differential/generics). **Self-host fixpoint BYTE-VERIFIED** (md5 `50857425ec4be97ddf971074a6b66d48`). Remaining cores before 0.1.0 (no-deferral policy, user directive 2026-08-28): 0.0.116 (audit close-off #2: builtin+live-local miscompile fix + quantum/linalg/trig gate tests + mtu wiring), 0.0.117 (`big` completion: ordering + bitwise ops), 0.0.118 (net send/recv), 0.0.119 (futex + threads), 0.0.120 (stack_trace full unwind), 0.0.121 (closures by-ref capture), 0.0.122 (extern "C" standalone EXE). Then 0.1.0 (std/lib resumes, borrow-checking, PTY layer). P4 tooling moved AFTER stdlibs (POST-0.1.0).
