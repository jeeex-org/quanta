...
> **Last updated: 2026-08-29. Current compiler: 0.0.120** (x86-64 ELF emitter,
...
**0.0.120 (current stable seed):** `stack_frames()` full stack unwind — walks the rbp frame chain and returns a qword-array of real return addresses (4-deep chain verified by `stack_frames_test.quanta`, rc=0). Uses `g_code_end` (CODE_VBASE+codelen, written by write_elf) to bound each candidate against the code segment, terminating at the entry stub's `rbp=argv` frame. No per-call instrumentation → fixpoint-safe. Remaining cores before 0.1.0 (no-deferral policy, user directive 2026-08-28): 0.0.121 (closures by-ref capture), 0.0.122 (extern "C" standalone EXE). Then 0.1.0 (std/lib resumes, borrow-checking, PTY layer). P4 tooling moved AFTER stdlibs (POST-0.1.0).
