...
| 0.0.123 | Lang | nested named-fn escape (closure fix) | ❌ todo | **Closes the one verified gap left after the 0.0.122 gate (every verified gap gets a version):** nested *named* `fn name(){}` defined inside another fn and called/returned errors `undeclared function: name` (reproduced 0.0.122: `fn make(){ let f = fn helper():i64 { return 7 }; return f() }` → `error: undeclared function: helper`). Root cause: the nested fn is scanned into the global fn table but not registered in the enclosing fn's local scope, so the inner call cannot resolve it. `\|x\|` lambda literals already work (closure path). Fix: register a nested named fn in the enclosing scope (like a local fn binding) and route its call/escape through the existing closure machinery (env capture of the enclosing frame). NOTE: multi-arg extern-C (e.g. `strcmp(a,b)`, already gated in `extern_c_ffi.quanta`) and the security KNOWN-issue items were investigated for 0.0.123 and found ALREADY WORKING (garbage-input fuzz = 0 compiler crashes; MININT-1 → clean rc=7 error) — not gaps. **Fixpoint:** 0.0.123 source must self-host stage1==stage2 byte-identical (seed 0.0.122). |

**Version-number rule:** literal numbers above are the plan; the SEQUENCING (order + one-feature-per-version + fixpoint-verified) is the contract. If a version needs splitting, the number increments — never skipped, never reused. 0.0.90 is **not reserved**; it is simply the `as`/`raw`/`volatile` cluster above. The final cores are 0.0.115 (security hardening, ✅), 0.0.116 (audit close-off #2), 0.0.117 (`big` completion), 0.0.118 (net send/recv), 0.0.119 (futex + threads), 0.0.120 (stack_trace full unwind), 0.0.121 (closures by-ref capture), 0.0.122 (extern "C" standalone EXE), 0.0.123 (nested named-fn escape fix) — **no-deferral policy to 0.1.0 (user directive 2026-08-28)**: every verified gap gets a version. **The code-writing tool is deferred to AFTER stdlibs (post-0.1.0)** — it is no longer the last 0.0.x item. ARM64 and P4 tooling remain POST-0.1.0 per earlier user directives.
