...
| 0.0.104 | Builtins | string ops (strcat/substr/strcmp/str_split/utf8) | 🟡 partial | `strcat`/`substr` in 0.0.102; `str_split` added in 0.0.104 (string-array runtime via `vreg_str_arr` tag, str_split_test GREEN). Remaining: `utf8` (and `strcmp` is really `str_eq`/`str_ne`, already shipped). |
