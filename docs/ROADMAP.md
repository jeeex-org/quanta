...
### Current status (0.0.114 — live)

**0.0.114 (current stable seed):** Lang — `big` first-class type keyword (moved from the stdlib track to core). Full 8-layer gate GREEN (147/147 functional + extern-c/security/perf/valgrind/fuzz/differential/generics-negative); stdlib suite 7/7 GREEN against the annotated `lib/std/big.quanta`. **Self-host fixpoint BYTE-VERIFIED** (md5 `637c7c694f04a7579468715c1f0c8b97`): the promoted binary compiles its own source to a byte-identical binary (multiple independent builds). The self-host chain — systemically broken (stage2 SIGSEGV) since 0.0.109 — is fully restored. All 0.0.x cores are CLOSED; next milestone is 0.1.0 (std/lib resumes, borrow-checking, PTY layer). P4 tooling moved AFTER stdlibs (POST-0.1.0).
