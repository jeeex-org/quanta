...
**0.0.131 (current stable seed, live):** closure self-recursion by name (partial core). A named closure body can now refer to its OWN name to recurse: `let fact = fn fact(n: i64): i64 { if n<=1 {return 1}; return n*fact(n-1) }`. The closure's self-name is bound as a real enclosing-scope local (type-11, exactly like `let f = fn..{}` does) **and** registered as a capture of that home slot, so the body reads the just-emitted closure tuple via `IR_CAPREAD` and the self-call `fact(n-1)` resolves via `vfind` to a type-11 local and routes through `IR_CLOSURE_CALL` (same path as `f(5)`). Before 0.0.131 the self-name was either unbound (`error: undeclared function: fact`) or, when force-bound to an undefined vreg, segfaulted at runtime (jumped to a garbage codeptr). New `closure_selfrec_test.quanta` (rc=0: fact(5)=120) gates it; existing closure tests (closure_named_fn rc=0, closure_basic rc=6, closure_capture rc=15) still GREEN. Built from 0.0.130 seed; fixpoint byte-verified (gen1==gen2==gen3, md5 `8e1bb23fc7e626ee4b8513dc690197c1`). Gate: functional **163/163**, stdlib 7/7, multi-TU 3/3, all 11 layers GREEN. Next core = **0.0.132** (`json`). 0.1.0 = post-core STABLE.
...
- 0.0.133 `secure` TLS 1.3 + hybrid X25519/ML-KEM (FIPS 203) — **PQC-ready**, the QC-age requirement
- 0.0.134 `quic` HTTP/3 (UDP+TLS 1.3, hybrid PQ) — modern transport, ahead of HTTP/2
- 0.0.135 `http` HTTP/2-over-TLS (no plaintext shipped)
- 0.0.136 `ai` tensor ops + inference (AI-age, promoted into core chain)
- 0.0.137 generics type constraints (FIX-0.0.33; `where` bounds enforced) — classical type-system
- 0.0.138 borrow-check (language safety pass) — core, hardest
