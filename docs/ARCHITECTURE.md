...
Last updated: 2026-08-22. Author: JEEEX.ORG. Status: x86-64 (AArch64 backend planned POST-0.1.0)
...
- `compiler/$(cat VERSION)/src/x86/` — multi-file x86-64 Quanta compiler (AArch64 backend planned POST-0.1.0)
  (main.quanta + helpers/lexer/parse/codegen/emitter/elf/globals/quanta).
  Self-hosts (3-stage: `compiler/$(cat VERSION)/bin/x86/qc` -> source -> `qc`, byte-identical fixed point).
- `compiler/$(cat VERSION)/bin/x86/qc` is the golden compiler; it compiles the $(cat VERSION)
...
1. Self-host: `cd compiler/$(cat VERSION)/src/x86; SEED=../../../../compiler/$(cat VERSION)/bin/x86/qc; $SEED main.quanta qc && ./qc main.quanta qc2 && ./qc2 main.quanta qc3` must produce byte-identical qc==qc2==qc3 (3-stage fixed point).
