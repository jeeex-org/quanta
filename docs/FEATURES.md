...
| `pki` (X.509/PKI/CT/OCSP) | ✅ **0.151 DONE** — X.509/PKI; **0.152 DONE** — Certificate Transparency; **0.153 DONE** — OCSP Stapling | ✅ gate (ocsp_const_test.quanta rc=0) | ASN.1 DER parser, cert chain validation, SAN/IP/CN, expiry, revocation (CRL/OCSP), trust store, cert/key PEM/DER load/save. CT: SCT parse (single/list), log_id/timestamp/sig, Merkle inclusion/consistency proofs, known log list, stapled SCT stub. OCSP: request/response parse, CertID/RevokedInfo/Good/Unknown, producedAt/thisUpdate/nextUpdate, nonce, stapling cache, sig verify stub. |
| `secure_ext` (TLS resumption, mTLS, HSM, hardening) | ✅ **0.154 DONE** — TLS Resumption; ✅ **0.155 DONE** — mTLS/SPIFFE; ✅ **0.156 DONE** — HSM/PKCS#11/TPM/SGX/SEV; ✅ **0.157 DONE** — Key Derivation/Keystore; ✅ **0.158 DONE** — Side-Channel Hardening; ✅ **0.159 DONE** — Audit Logging/Sigstore/SBOM; ✅ **0.160 DONE** — Supply Chain/Policy/Graph; ✅ **0.161 DONE** — Compliance (FIPS 140-3/CC EAL4+/ISO 27001/NIST 800-53/SOC 2/GDPR/CCPA/CNSA 2.0); ✅ **0.162 DONE** — Policy Engine (Rego subset, TLS/cert/cipher/mTLS/HSM/supply-chain/compliance policies, audit logging); ❌ todo → **0.163+ core** | ✅ gate (policy_engine_const_test.quanta rc=0) | Session tickets (RFC 5077), PSK (RFC 8446), 0-RTT with replay protection. Ticket encrypt/decrypt (AES-256-GCM), key rotation, PSK binder computation, early secret derivation, 0-RTT replay cache, ticket serialize/deserialize, OCSP stapling integration. mTLS/SPIFFE: SPIFFE ID parse/build/validate, X.509-SVID cert SAN extraction, JWT-SVID sub claim parse, SVID create/verify/rotate, trust domain/bundle/federation/workload management, SPIRE agent API, mTLS peer verification. HSM: PKCS#11 full API (slots/tokens/sessions/objects, login, key gen for ML-KEM/ML-DSA/SLH-DSA), TPM 2.0 ESAPI (create/load/sign/quote/attest), SGX/SEV attestation (report/quote), HSM abstraction layer (unified backend, key import/export, cert storage, key derivation, health check). Key Derivation: PBKDF2-HMAC-SHA256 (RFC 8018), HKDF-Extract/Expand (RFC 5869), Scrypt (RFC 7914), Argon2id (RFC 9106), encrypted keystore with AES-256-GCM + Argon2id, key rotation policies (daily/weekly/monthly/yearly), secure delete (3-pass overwrite). Side-Channel: ct_memcmp/ct_eq32/ct_eq64/ct_select32/ct_cond_copy/ct_memcpy/ct_memset, cache-line aligned alloc/prefetch/flush, masked XOR/AND/sharing (ISW), ML-KEM/ML-DSA/SLH-DSA masked keygen/sign/verify, secure alloc/pool, timing test harness. Audit: Merkle tree log (RFC 6962) with leaf/node hashes, tree head signing (ML-DSA), inclusion/consistency proofs, Trillian-compatible API, sigstore bundle create/verify, cosign keyless sign/verify, in-toto/SLSA provenance, SPDX/CycloneDX SBOM, reproducible build attestation. Supply Chain: In-toto layout/link verification, SLSA provenance verify (levels 0-3), sigstore/cosign keyless verify, transparency log inclusion, SPDX/CycloneDX SBOM verify/diff, reproducible build fingerprint/verify, attestation create/verify, OPA-like policy eval/compile, dependency graph build/paths/cycles. Compliance: FIPS 140-3 CM, CC EAL4+, ISO 27001, NIST 800-53, SOC 2, GDPR, CCPA, CNSA 2.0. Policy Engine: Rego subset parser (package/import/default/rules/functions), native evaluator for TLS config (min/max version, AEAD ciphers, curves, cert/key), cert pinning (SPKI SHA-256 matching), cipher suites (strict/permissive mode, deny list for weak suites, allow list for TLS 1.3), mTLS (client cert, hostname verify, trust domains, SPIFFE IDs), HSM key usage (PQC keys hw-only/non-extractable, op restrictions), supply chain (SBOM required, SLSA level >= 2, sigstore verify), compliance (framework status/evidence), built-in policy bundle, decision logging to audit log. |
| `ai_llm` (LLM-specific: RoPE/RMSNorm/SwiGLU/GQA, KV cache, FlashAttn, BF16, quantization, distributed training, tokenizers) | ❌ todo → **0.163–0.172 core** | ❌ none | RoPE, RMSNorm, SwiGLU/GeGLU, GQA, KV cache + speculative decoding, FlashAttention, BF16/FP8, mixed precision, gradient checkpointing, ZeRO/FSDP, tensor/model/pipeline parallelism, NCCL/all-reduce, tokenizers (BPE/WordPiece/TikToken), LR schedulers (cosine/warmup), weight tying, model zoo (LLaMA/Gemma/Qwen/Mistral/Phi), vLLM-style inference engine (continuous batching, paged attention), ONNX export. |
