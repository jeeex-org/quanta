...
| `secure_ext` (TLS resumption, mTLS, HSM, hardening) | ✅ **0.154 DONE** — TLS Resumption; ✅ **0.155 DONE** — mTLS/SPIFFE; ✅ **0.156 DONE** — HSM/PKCS#11/TPM/SGX/SEV; ❌ todo → **0.157–0.158 core** | ✅ gate (hsm_const_test.quanta rc=0) | Session tickets (RFC 5077), PSK (RFC 8446), 0-RTT with replay protection. Ticket encrypt/decrypt (AES-256-GCM), key rotation, PSK binder computation, early secret derivation, 0-RTT replay cache, ticket serialize/deserialize, OCSP stapling integration. mTLS/SPIFFE: SPIFFE ID parse/build/validate, X.509-SVID cert SAN extraction, JWT-SVID sub claim parse, SVID create/verify/rotate, trust domain/bundle/federation/workload management, SPIRE agent API, mTLS peer verification. HSM: PKCS#11 full API (slots/tokens/sessions/objects, login, key gen for ML-KEM/ML-DSA/SLH-DSA), TPM 2.0 ESAPI (create/load/sign/quote/attest), SGX/SEV attestation (report/quote), HSM abstraction layer (unified backend, key import/export, cert storage, key derivation, health check). Remaining: key derivation (PBKDF2/Argon2/Scrypt), encrypted keystore, constant-time comparators, masking/blinding, formal verification hooks, audit logging, supply chain (sigstore), compliance (FIPS 140-3/CC). |
