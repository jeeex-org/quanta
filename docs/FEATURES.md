...
| `!` / `not` | ⚠️ INCONSISTENT | 🟡 partial | bitwise_not rc=0 gates the CONSTANT path (`!0`=-1, `!1`=-2, `!!7`=7). But the RUNTIME path is logical: `let a=0; !a` → **1**, not -1. Same expression, two semantics depending on whether the operand const-folds. Pre-existing (present in 0.0.69); found by a 0.0.70 audit. Needs its own version + a semantic decision (pick bitwise or logical, or split into two operators) |
