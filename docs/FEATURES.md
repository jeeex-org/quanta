...
| `secure_ext` (TLS resumption, mTLS, HSM, hardening) | ✅ **0.154 DONE** — TLS Resumption; ❌ todo → **0.155–0.158 core** | ✅ gate (tls_resumption_const_test.quanta rc=0) | Session tickets (RFC 5077), PSK (RFC 8446), 0-RTT with replay protection. Ticket encrypt/decrypt (AES-256-GCM), key rotation, PSK binder computation, early secret derivation, 0-RTT replay cache, ticket serialize/deserialize, OCSP stapling integration. Remaining: mTLS/SPIFFE, TPM/PKCS#11/SGX, key derivation (PBKDF2/Argon2/Scrypt), encrypted keystore, constant-time comparators, masking/blinding, formal verification hooks, audit logging, supply chain (sigstore), compliance (FIPS 140-3/CC). |
