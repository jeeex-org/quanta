...
| `$$(cmd)` external command | ✅ done (0.0.74) | 🟡 1 test | CAPTURE-ONLY (no interactive/TTY support). String `$$("cmd")`, array `$$(["cmd","arg"])`, raw `$$(cmd arg)`. Returns CmdResult{out,code,err}. Gated behind `unsafe{}`. Linux-only (raw fork/execve syscalls). PTY layer (for vi/ssh/top) is 1.0 work |
