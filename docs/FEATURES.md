...
| `$$(cmd)` external command | ✅ done (0.0.73) | 🟡 no gate | string, array, AND no-quote forms. `$$("echo hi")`, `$$(["/bin/echo","hi"])`, `$$ ls -l ~/ `. Returns CmdResult{out, code, err}. Gated behind `unsafe {}`. No-quote form added in 0.0.73 |
