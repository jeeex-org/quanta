...
| security KNOWN issues (compiler robustness) | ❌ todo (0.0.123) | ❌ none | Gate prints `KNOWN` (non-blocking) items, reproduced on 0.0.122: (a) **garbage-input fuzz crashes the COMPILER** — `gf_crashes` of `gf_total` crafted inputs SIGILL/SIGSEGV the `qc` binary (compiler-robustness bug, originally tagged "fix in 0.0.47", still open); (b) **compiler SIGILL on extreme literal MININT-1** — an out-of-range integer literal makes the lexer/compiler fault instead of a clean lexer ERROR. Both are compiler-crashes-on-bad-input, not miscompiles of valid programs. 0.0.123 re-triages each as blocking/non-blocking and fixes the crashes (fail-closed: clean error, never a SIGILL/SIGSEGV). |
