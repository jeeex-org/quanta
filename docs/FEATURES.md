...
| security KNOWN issues (compiler robustness) | ❌ none (verified 0.0.122) | ❌ none | Investigated for 0.0.123 and found ALREADY WORKING on 0.0.122: the security gate reports **0 known issues**; garbage-input fuzz = **0 compiler crashes**; extreme literal MININT-1 → clean lexer ERROR (rc=7), not a SIGILL. (The SIGILL lines printed by `security_tests.sh` are the *generated test binaries* trapping on overflow — expected PASS, not compiler faults.) Multi-arg extern-C and these security items are therefore NOT gaps. The only remaining verified gap is the nested named-fn escape (0.0.123). |
...
| atomics | atomic_load/store/add/swap/cmpxchg | ✅ 5/5 gated (atomic_test.rc=11). **futex + thread create/join shipped 0.0.119** (`futex_wait`/`futex_wake` sc 202; `thread_create(fn,arg)`/`thread_join(tid)` via clone sc 56 + per-thread 1MB stack mmap + join-slot mmap; CLONE_CHILD_CLEARTID wakes join on exit). **Hardened in 0.0.124** (AUDIT_ROADMAP Part D): join-slot + child-stack `mmap` now have MAP_FAILED guards (FIX-0.0.35/36), `futex_wait`/`futex_wake` clamp negative-errno → 0 (FIX-0.0.37, verified `futex_wake(0,1)` rc 242→0), and `clone` failure now `munmap`s both mappings + `exit(1)` instead of leaking (FIX-0.0.38). Findings 39/40/41/42 re-verified as stale/not-defects. `thread_test.quanta` (worker(7)=107) + `futex_test.quanta` GREEN. |
