...
|| `secure` (TLS 1.3, PQC-ready, constant-time) | ✅ **0.0.135 DONE** — full FIPS 202 + AES-GCM | ✅ gate | `std_secure_sha3_test.quanta` (rc=0), `std_aes_gcm_test.quanta` (rc=0); modular: sha3_constants, sha3_theta, sha3_rhopi (fixed Pi perm), sha3_chi, sha3_keccak_f, sha3_hash, aes_gcm. 6 FIPS 202 funcs: sha3_224/256/384/512, shake128/256 + AES-128-GCM (enc/dec). All KATs pass via direct compile (`--no-overflow-trap`) + object mode. **Note:** `--no-overflow-trap` is the intended flag for crypto/unsigned-bitwise code (not a workaround) — the signed-overflow trap fires on intentional two's-complement wraparound (rotations, XOR, shifts, byte-positioning multiplies like `(j & 7) * 8`). The shift-count UB trap (`shl_trap_emit`, count ≥ 64) remains active.
