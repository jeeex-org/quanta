...
1. **Self-host fixed point**: 3-stage bootstrap
   `qc-bootstrap-0.0.45 → qc_boot → qc_self → qc`, all rc=0,
   rebuilt `qc` byte-identical to committed `qc`. (A miscompiling
...
- No requirements traceability matrix (each IR op / builtin → requirement
  → test).
- No documented verification plan beyond the 81-test gate.
