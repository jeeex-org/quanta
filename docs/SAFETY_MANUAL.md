...
- Interpreter / JIT / WASM backends (not yet built).
...
| Documentation | Tool spec + safety manual | **SPEC.md (v0.0.169) + this manual exist** |
...
| Independent implementation | Cross-check tool | **PARTIAL** — differential vs bootstrap-SEED qc (5/5 parity, 0.0.169); full x86↔ARM64 differential is POST-0.1.0 (docs/roadmap/quanta.md §Standards) |
...
Quanta follows a **"fail-secure, not prove-safe"** model: undefined/unsafe operations trap at runtime rather
...
| Executable output | ELF mode 0755 | Verified 0.0.169 (binary runs directly) |
...
1. **Self-host fixed point**: 2-stage bootstrap
   `committed compiler/$(cat VERSION)/bin/x86/qc → $(cat VERSION) source → qc`, all rc=0,
...
4. **Grammar / static-analysis readiness**: all 15 compiler modules
   parse with **0 errors** under `tree-sitter-quanta` at v0.0.169
...
5. **Latent-defect caught**: keyword-hash constants `H_ENUM`/`H_MUT`/
   ...
   v0.0.169. This is exactly the class of defect a safety process must
   catch (see §5).
...
| S-004 | keyword-hash paren imbalance → wrong lexer hash | **High (silent corruption)** | **FIXED 0.0.169** |
| S-005 | emitted binary missing exec bit | Med (usability) | **FALSE ALARM** — mode 0755 confirmed |
| S-006 | grammar module-parse cascade (~1000/file) | Med (no CI review) | **FIXED 0.0.169** (0 errors) |
...
qualification requires a precise spec. Partial: `docs/SPEC.md` v0.0.169
...
need compile-time guarantees (borrow checking — SPEC.md §8, NOT yet built).

### 6.4 Process artifacts (MED)
- No requirements traceability matrix (each IR op / builtin → requirement
  → test).
- No documented verification plan beyond the 110-test gate.
...
1. **Close S-006 grammar** (done 0.0.169) → enables CI static analysis.
...
Manual corresponds to compiler `0.0.169`.
