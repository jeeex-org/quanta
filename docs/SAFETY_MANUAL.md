...
| Validation | Demonstrate correct behavior | Partial: self-host fixed-point (2-stage, byte-identical) + 99-test gate + fail-closed tests (§4) |
...
1. **Self-host fixed point**: 2-stage bootstrap
   `committed bin/x86/qc → 0.0.66 source → qc`, all rc=0,
...
- No requirements traceability matrix (each IR op / builtin → requirement
  → test).
- No documented verification plan beyond the 99-test gate.
