...
This document describes how Quanta is designed to do exactly that. It is not a
proposal to replace anything — it is Quanta's own design philosophy, written out
in full, with concrete syntax, a concrete type system, and honest tradeoffs.
Where it overlaps with what Quanta already ships (simple syntax, multiple
execution modes, self-hosting), that is the design already made real.
...
Quanta syntax is **spatial, not ceremonial**. There is no `public static void main(String[] args)`.
There is no boilerplate to "earn the right" to print. The top of a file is a
program.

```
# This is a complete, runnable Quanta program.
println "Hello, world"
```

Indentation defines structure (like Python), but Quanta forbids the classic
...
everywhere. This avoids the "everyone must learn the borrow checker" problem
...
A Quanta compile error is a *conversation*, not a verdict:
...
error: `age` cannot be negative here.
  ...
  hint: similar code at examples/age-guard.qx works.
```
...
Quanta ships **dialects** — vocabularies with first-class syntax sugar — for
...
```
quanta build --target native      # your laptop
quanta build --target wasm        # the browser
quanta build --target gpu         # a shader / kernel
quanta build --target mcu         # a microcontroller (region model shrinks to RAM)
...
vs Rust problem vs C problem. There is Quanta, at the rung your problem needs.
...
**AI-native by construction.** Quanta's source of truth is its AST, not raw text.
...
tree. (This is the natural home for Quanta's native "code-writing tool" — a
language the machine can edit without breaking.)
...
- **Your language, your keywords.** `println`, `mostrar`, `显示`, `පෙන්වන්න` are all
...
- **Non-typing entry.** Voice and visual block interfaces compile to the *same*
  AST, so someone who cannot type still writes first-class Quanta.

---

## 7. The Hard Parts (Honest Tradeoffs)

A vision that hides its costs is a fantasy. Here is where Quanta pays:
...
- **Gradual typing has a dynamic edge.** Fully untyped code paths rely on runtime
  checks. Quanta makes those checks *visible and cheap*, but they exist. Hot loops
...
- **Dialects risk fragmentation.** If every domain invents notation, readability
  across disciplines suffers. Quanta governs dialects: they must be registered,
...
- **Compile time.** Inference + refinement + multi-target codegen is not free.
  Quanta invests in incremental compilation and a fast self-hosted compiler so the
...
## 9. Where Quanta Is Today

This document is the full design. Quanta does not implement all of it yet — and
that is by design, not by accident. The language is built incrementally, one
feature per version, gate-green before promotion. The current state (0.0.86) and
the sequenced plan to reach the fuller design live in `docs/ROADMAP.md` and
`docs/QUANTA_GAP_ANALYSIS.md`.

What is already real today: a self-hosting native compiler with a byte-identical
fixpoint; a low-ceremony inferred syntax (Rung 0–2 of the ladder); a primitive
layer (syscalls, mmap, file I/O, floats, closures, generics, big-int); and
differentiating stdlib seeds (`crypto`, `quantum`, `linalg`, `math`). What remains
is architectural — region/ownership memory, multi-backend codegen, dialect
notation, package management, and the tutor/accessibility experience layer.

The direction is constant: **lower the floor without lowering the ceiling.** Quanta
is the implementation of this design, one version at a time.
